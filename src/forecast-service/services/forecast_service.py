"""
Core forecasting logic: Prophet-based time-series prediction with heuristic fallback.
"""
...
def _historical_balance_series(daily: pd.DataFrame, starting_balance: float) -> pd.DataFrame:
    frame = daily.copy()
    frame["y"] = starting_balance + frame["amount"].cumsum()
...
def run_scenario(base_forecast: Dict, scenario: Dict) -> Tuple[Dict, Dict, Dict]:
    ...
    low = {
        "forecast_data": shifted_rows(-0.15),
        "confidence": max(35, base_forecast.get("confidence", 70) - 8),
    }
    likely = {
        "forecast_data": shifted_rows(1.0),
